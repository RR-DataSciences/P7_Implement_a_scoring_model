...
def train_models(df, data_name):
    ...
    #     # Active l'autologging pour toutes les bibliothèques prises en charge
    #     mlflow.autolog()
    ...
    #     with mlflow.start_run(run_name=name_prefix):
      ...
      rfe = RFE(estimator=model, step=1, n_features_to_select=0.3, verbose=3, importance_getter='auto')
      ...
      column_names = X_train.columns[rfe.support_].tolist()
      pd.DataFrame(X_train_rfe).to_csv('/content/drive/MyDrive/OC_Data_Scientist/P7_Implémentez_un_modèle_de_scoring/X_train_rfe.csv', sep=';', index=False, header=column_names)
      column_names = X_test.columns[rfe.support_].tolist()
      pd.DataFrame(X_test_rfe).to_csv('/content/drive/MyDrive/OC_Data_Scientist/P7_Implémentez_un_modèle_de_scoring/X_test_rfe.csv', sep=';', index=False, header=column_names)
    ...
    #   mlflow.log_metric("best_Threshold", round(model_threshold.best_threshold_, 3))
      ...
      model_filename = f"{model_save_path}/{name_prefix}.dill"
    ...
    #   print(f"Modèle enregistré localement sous {model_filename}")
      ...
      # Enregistrement local des données transformées par RFECV
      rfe_filename = f"/content/drive/MyDrive/OC_Data_Scientist/P7_Implémentez_un_modèle_de_scoring/models/rfe_{name_prefix}.dill"
      with open(rfe_filename, 'wb') as f:
          dill.dump(rfe, f)
      print(f"RFECV enregistré localement sous {rfe_filename}")
    ...
    return X_train_rfe, X_test_rfe, y_train, y_test, rfe, grid_search.best_estimator_
